...
class ReachingDefinitions(Analysis):
    ...
    def analyse(self, step):
        ...
        if type(step[1]) == VariableAssignmentStatement or type(
                step[1]) == VariableDeclaration:  # If type variable assignment
            ...
            genSet = (step[1].getName(), step[
                1].label)  # create a genSet based on the name of variable and the label of the node we're going to
